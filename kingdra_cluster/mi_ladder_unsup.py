...
def get_mi_ladder_model( inp_dim ,
    ...
    corr_logits_sup = Lambda(lambda x: x[0])([ corr_logits_sup ,  clean_logits ,corr_logits  , clean_logits_sup  , u , z_est , z ])
    ...
    noise_kl = tf.reduce_mean(compute_kld( tf.stop_gradient(clean_logits) , corr_logits  ))

    tot_loss = ladder_loss_w*ladder_cost + mi_loss_w*(mi_loss_clean + mi_loss_corr) + noise_kl_w*noise_kl
    ...
    model.compile( optimizer ,  'categorical_crossentropy' , loss_weights=[ sup_loss_w ]  ,  metrics=[ ] )
...
class LadderIM(object):
    ...
    def fit( self , X_train ):
        ...
        for it in range(   n_iter ):
            ...
            model.fit(  x=[ X_train, X_train ] , y=Y_train , batch_size=batch_size , epochs=n_epochs_per_iter  , shuffle=False  )
