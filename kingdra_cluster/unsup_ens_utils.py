...
def get_en_clus( t_low ,t_med, t_high , all_outputs, thresh=0.0 ):
    ...
    for i in range( en_inds.shape[0]):
        ss2 = ((en_inds == en_inds[i]) & ((max_outputs_t[i] > thresh).sum() == n_en)).sum(-1)
...
def calc_ensemble_acc( all_outputs, buckets, m_inds, y_true ):
    ...
    all_outputs = np.asarray([all_outputs[i,:,m_inds[i]].T for i in range (n_en)])
    ...
    y_pred = np.asarray([np.bincount(max_outs[:,i]).argmax() for i in range(n_points)])
    y_pred = map_cl[y_pred]
    n_agree = np.sum(y_true == y_pred)
