...
class KingdraCluster(object):
	...
	def fit( self  , X_train , callback=None  , cb2=None ):
		...
		models = [ get_mi_ladder_model( X_train.shape[-1] ,  **model_args ) for k in range(n_en) ]
		...
		for it in range(   n_iter ):
			...
			for en in range( n_en ):
				...
				if it == 0 or min_labels == 0:
					...
					X_train_sup = X_train
					model.fit(  x=[X_train , X_train_sup] , y=Y_train_sup , batch_size=batch_size , epochs=n_epochs_per_iter0  , shuffle=False, verbose=2) #) callbacks=[lrate]  )
				else:

					if not ens_common_mapping:
						m_buckets = get_bucket_modelwrt_ind( buckets , all_outputs[en].argmax(-1))

					X_train_sup , Y_train_sup  =  get_training_from_buckets( m_buckets , X_train, it,  smooth)

					model.fit(  x=[X_train , X_train_sup] , y=Y_train_sup , batch_size=batch_size , epochs=n_epochs_per_iter  , shuffle=True, verbose=2) #) callbacks=[lrate]  )
			...
			buckets = get_en_clus( low_th , med_th, high_th, all_outputs1, thresh )
	...
	def load_weights(self , path ):
		...
		self.models = [ get_mi_ladder_model( inp_l ,  **model_args ) for _ in range(n_en) ]
